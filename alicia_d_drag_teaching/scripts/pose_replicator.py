...
class PoseReplicator:
    ...
    def load_trajectory_from_bag(self):
        ...
        try:
            # 打开bag文件并重建索引
            bag = rosbag.Bag(self.bag_file_path, 'r', allow_unindexed=True)
    ...
    def run(self):
        ...
        # 检查是否是自动执行模式（由pose_record.py启动）
        auto_execute = rospy.get_param('~auto_execute', False)
        
        if auto_execute:
            rospy.loginfo("自动执行模式，立即开始执行轨迹...")
            rospy.sleep(1.0)  # 短暂延迟确保系统准备就绪
        else:
            # 提示用户开始执行
            ...
            try:
                ...
                if sys.stdin.isatty():
                    ...
                    rospy.loginfo("检测到非交互模式，自动开始执行...")
                    rospy.sleep(1.0)
            except (EOFError, KeyboardInterrupt):
                rospy.loginfo("输入被中断，自动开始执行...")
                rospy.sleep(1.0)
            except Exception as e:
                rospy.logwarn("获取用户输入失败: %s，自动开始执行...", str(e))
                rospy.sleep(1.0)
